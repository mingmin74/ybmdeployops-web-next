...
export function getHaStatus() {
  return request<PveRecord[]>('/api2/json/cluster/ha/status/current', { method: 'GET', notifyOnError: true });
}
...
export function getHaResource(id: string) {
  return request<PveRecord>(`/api2/extjs/cluster/ha/resources/${encodeURIComponent(id)}`, { method: 'GET' });
}
...
export function updateHaResource(id: string, data: PveRecord) {
  return request(`/api2/extjs/cluster/ha/resources/${encodeURIComponent(id)}`, { method: 'PUT', data });
}

export function deleteHaResource(id: string) {
  return request(`/api2/extjs/cluster/ha/resources/${encodeURIComponent(id)}`, { method: 'DELETE' });
}

export function getHaRules() {
  return request<PveRecord[]>('/api2/json/cluster/ha/rules', { method: 'GET', notifyOnError: true });
}
