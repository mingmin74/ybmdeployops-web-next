...
const columns = computed(() => [
  ...
  {
    name: 'snaptime',
    label: gettext('Snapshot time'),
    field: 'snaptime',
    align: 'left' as const,
    format: (value: unknown) => formatTime(value),
  },
  {
    name: 'vmstate',
    ...
    field: 'vmstate',
    align: 'left' as const,
    format: (value: unknown) => (Number(value) === 1 ? gettext('Yes') : gettext('No')),
  },
...
function confirm(nextAction: 'rollback' | 'delete', row: PveRecord) {
  if (
    ...
    (nextAction === 'delete' && !canSnapshot.value)
  )
    return;
...
async function openEdit(row: PveRecord) {
  ...
  try {
    ...
    selected.value = row;
    editDescription.value = textValue(response.data?.description) || textValue(row.description);
...
async function runAction() {
  ...
  try {
    ...
    emit(
      ...
      action.value === 'rollback' ? gettext('Rollback') : gettext('Delete'),
    );
    await reload();
  } finally {
    loading.value = false;
...
<template>
  <div class="vm-snapshots-tab q-pa-md">
      ...
      <template #body-cell-actions="scope">
        <q-td :props="scope">
          <q-btn
            ...
            @click="confirm('rollback', scope.row)"
          />
          <q-btn
            no-caps
            flat
          ...
          <q-btn
            ...
            @click="confirm('delete', scope.row)"
          />
        </q-td>
      </template>
    </q-table>
    ...
    <q-dialog v-model="editVisible" persistent
      ><UWindow :title="gettext('Edit snapshot')" width="520px" :loading="loading"
        ><div class="q-pa-md">
          <q-input
        ...
        <template #foot
          ><q-btn
            ...
            class="bg-primary text-grey-1 u-button"
            :label="gettext('Save')"
            @click="saveEdit" /></template></UWindow
    ></q-dialog>

    <q-dialog v-model="actionVisible" persistent>
      <UWindow
        :title="action === 'rollback' ? gettext('Rollback') : gettext('Delete')"
        width="480px"
        :loading="loading"
      >
        <div class="q-pa-md">
          {{
            action === 'rollback'
              ? gettext('Rollback this virtual machine to the selected snapshot?')
              : gettext('Delete the selected snapshot?')
          }}
        </div>
        <template #foot>
          <q-btn
