...
async function loadLogs() {
  loading.value = true;
  try {
    const response = await getVmFirewallLogs(props.node, props.vmid, { start: 0, limit: 500 });
    const payload = response.data;
    logs.value = Array.isArray(payload) ? payload : payload?.data || [];
  } finally {
    loading.value = false;
  }
}
...
<template>
  <div class="vm-firewall-tab q-pa-md">
    <q-tabs v-model="section" dense align="left" active-color="primary" indicator-color="primary"
      ><q-tab name="rules" :label="gettext('Rules')" /><q-tab
        name="options"
        :label="gettext('Options')" /><q-tab name="aliases" :label="gettext('Alias')" /><q-tab
        name="ipset"
        :label="gettext('IPSet')" /><q-tab
        v-if="canViewFirewallLog"
        name="log"
        :label="gettext('Log')"
        @click="loadLogs" /></q-tabs
    ><q-separator />
        ...
        ><q-table
          ...
          ><template #top
            ><q-btn
              ...
              @click="removeRule" /><q-space /><q-btn
              no-caps
              outline
              size="12px"
              color="primary"
        ...
        ><q-markup-table flat
          ><tbody>
            <tr>
              ...
              <td class="text-right">
                <q-toggle
                  :model-value="Number(options.enable)"
                  :true-value="1"
                  :false-value="0"
                  :disable="!canConfigureFirewall"
                  color="primary"
                  @update:model-value="saveOption('enable', $event)"
                />
              </td>
            </tr>
            <tr>
              ...
              <td class="text-right">
                <q-popup-edit
                  :model-value="String(options.log_level_in || 'nolog')"
                  :disable="!canConfigureFirewall"
                  buttons
                  @save="saveOption('log_level_in', $event)"
                  v-slot="scope"
                  ><q-select
                    v-model="scope.value"
                    dense
                ...
                >{{ options.log_level_in || gettext('Default') }}
              </td>
            </tr>
            <tr>
              <td>log_level_out</td>
              <td class="text-right">
                <q-popup-edit
                  :model-value="String(options.log_level_out || 'nolog')"
                  :disable="!canConfigureFirewall"
                  buttons
                  @save="saveOption('log_level_out', $event)"
                  v-slot="scope"
                  ><q-select
                    v-model="scope.value"
                    dense
                ...
                >{{ options.log_level_out || gettext('Default') }}
              </td>
            </tr>
        ...
        <q-markup-table flat bordered dense class="full-width"
          ><thead>
            <tr>
              ...
              <th class="text-left">{{ gettext('Log') }}</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="(item, index) in logs" :key="index">
              <td>{{ item.time || '-' }}</td>
              <td class="firewall-log-line">{{ item.t || item.msg || item }}</td>
            </tr>
          </tbody></q-markup-table
        ></q-tab-panel
      >
    ...
    <q-dialog v-model="groupVisible" persistent>
      <UWindow :title="gettext('Insert Security Group')" width="460px" :loading="loading">
        <div class="q-pa-md q-gutter-sm">
          <q-select v-model="groupForm.action" dense square outlined emit-value map-options :options="securityGroupOptions" :label="gettext('Security Group')" :rules="[(value) => !!value || gettext('Required field')]" />
          <q-input v-model="groupForm.iface" dense square outlined :label="gettext('Interface')" />
          <q-checkbox v-model="groupForm.enable" :true-value="1" :false-value="0" :label="gettext('Enable')" />
          <q-input v-model="groupForm.comment" dense square outlined :label="gettext('Comment')" />
        </div>
