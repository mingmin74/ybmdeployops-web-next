...
const monitorLines = shallowRef<string[]>([gettext("Type 'help' for help.")]);
...
const cpuSeries = computed(() => [
  {
    ...
    data: rows.value.map((item) => Number(item.cpu || 0) * 100),
    color: '#1976d2',
  },
]);
const memorySeries = computed(() => [
  {
    name: gettext('Total'),
    data: rows.value.map((item) => Number(item.maxmem || 0)),
    color: '#8c96a8',
  },
  {
    name: gettext('RAM Used'),
    data: rows.value.map((item) => Number(item.mem || 0)),
    color: '#2e7d32',
  },
]);
const networkSeries = computed(() => [
  {
    name: gettext('NetIn'),
    data: rows.value.map((item) => Number(item.netin || 0)),
    color: '#00838f',
  },
  {
    name: gettext('NetOut'),
    data: rows.value.map((item) => Number(item.netout || 0)),
    color: '#1976d2',
  },
]);
const diskSeries = computed(() => [
  {
    name: gettext('Disk Read Speed'),
    data: rows.value.map((item) => Number(item.diskread || 0)),
    color: '#ef6c00',
  },
  {
    name: gettext('Disk Write Speed'),
    data: rows.value.map((item) => Number(item.diskwrite || 0)),
    color: '#c62828',
  },
]);
...
async function executeMonitorCommand() {
  const command = monitorCommand.value.trim();
  if (!command || monitorLoading.value || !props.node || !props.vmid) return;
  monitorCommand.value = '';
  monitorLines.value = [...monitorLines.value, `# ${command}`].slice(-5000);
  monitorLoading.value = true;
  try {
    const response = await runVmMonitorCommand(props.node, props.vmid, command);
    const output = String(response.data || '');
    monitorLines.value = [...monitorLines.value, ...output.split('\n')].slice(-5000);
  } finally {
...
watch(
  () => [props.node, props.vmid, timeframe.value],
  () => {
    void reload();
  },
  { immediate: true },
...
<template>
  <div class="vm-monitor-tab q-pa-md">
    ...
    <q-card flat bordered square class="q-mb-md">
      <q-card-section class="q-pb-sm text-weight-medium text-caption">{{
        gettext('QEMU Monitor')
      }}</q-card-section>
      <q-card-section class="q-pt-none">
        <pre class="vm-qmp-output">{{ monitorLines.join('\n') }}</pre>
        <div class="row q-gutter-sm">
          <q-input
            ...
            @keyup.enter="executeMonitorCommand"
          /><q-btn
            no-caps
            outline
            size="12px"
            ...
            :disable="!monitorCommand.trim()"
            @click="executeMonitorCommand"
          />
        </div>
      </q-card-section>
    </q-card>
    ...
    <div class="row q-col-gutter-sm">
      <div class="col-12 col-lg-6">
        <q-card flat bordered square
          ><q-card-section class="q-pb-none text-weight-medium text-caption">{{
            gettext('CPU Usage')
          }}</q-card-section
          ><LineMetricChart :x-data="xData" :series="cpuSeries" unit-type="percent" :height="250"
        /></q-card>
      </div>
      <div class="col-12 col-lg-6">
        <q-card flat bordered square
