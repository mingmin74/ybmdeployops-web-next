...
function payload() {
  ...
  for (const key of ['search', 'dns1', 'dns2', 'dns3'] as const) {
    ...
    else if (dns.value[key] !== undefined) deleted.push(key);
  }
...
async function save() {
  ...
  saving.value = true;
  try {
    await updateNodeDns(props.node, payload());
    visible.value = false;
    await load();
  } finally {
    saving.value = false;
    ...
    >
      ...
      >
        <q-input
          ...
          :label="gettext('Search domain')"
        />
        <q-input
          v-model="form.dns1"
          dense
          class="q-field--with-bottom"
          :label="`${gettext('DNS server')} 1`"
        />
        <q-input
          v-model="form.dns2"
          dense
          class="q-field--with-bottom"
          :label="`${gettext('DNS server')} 2`"
        />
        <q-input
          v-model="form.dns3"
          dense
          class="q-field--with-bottom"
          :label="`${gettext('DNS server')} 3`"
        />
      </q-form>
      <template #foot>
        <q-btn
