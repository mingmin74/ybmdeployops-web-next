...
const actions = computed(() => [
  {
    name: 'smart',
    label: gettext('Show S.M.A.R.T. values'),
    requiresSelection: true,
    disable: (row?: PveRecord) => Boolean(row?.parent),
  },
  {
    name: 'gpt',
    label: gettext('Initialize Disk with GPT'),
    requiresSelection: true,
    disable: (row?: PveRecord) => Boolean(row?.parent || (row?.used && row.used !== 'unused')),
  },
  {
    name: 'wipe',
...
const columns: QTableColumn<PveRecord>[] = [
  { name: 'devpath', label: gettext('Device'), field: 'devpath', align: 'left', sortable: true },
  {
    name: 'type',
    label: gettext('Type'),
    field: (row) => diskType(row['disk-type']),
    align: 'left',
    sortable: true,
  },
  {
  ...
  },
  {
    name: 'gpt',
    label: 'GPT',
    field: (row) => (row.gpt ? gettext('Yes') : gettext('No')),
    align: 'left',
    sortable: true,
  },
  ...
  {
    name: 'wearout',
    label: gettext('Wearout'),
    field: (row) =>
      Number.isFinite(Number(row.wearout)) ? `${100 - Number(row.wearout)}%` : 'N/A',
    align: 'left',
    sortable: true,
  },
];
...
function diskType(value: unknown) {
  return (
    ({ ssd: 'SSD', hdd: gettext('Hard Disk'), usb: 'USB' } as Record<string, string>)[
      String(value)
    ] || textValue(value, '-')
  );
}
...
async function loadRows(node: string) {
  ...
  const records: PveRecord[] = (response.data || []).map((item, index) => ({
    ...item,
    node,
  ...
  records.forEach((item) => {
    if (!item.parent) disks.set(String(item.devpath), item);
  });
  records.forEach((item) => {
    if (Array.isArray(item.partitions)) {
      item.children = (item.partitions as PveRecord[]).map((partition) => ({
        ...partition,
        node,
        parent: item.devpath,
        'disk-type': 'partition',
        used: partition.used === 'filesystem' ? partition.filesystem : partition.used,
        children: [],
      }));
    }
    if (item.parent)
      (disks.get(textValue(item.parent))?.children as PveRecord[] | undefined)?.push(item);
  });
  return [...disks.values()];
}

function diskName(row?: PveRecord) {
  return textValue(row?.devpath || row?.name || row?.device);
}
function nodeName(row?: PveRecord) {
  return textValue(row?.node || props.node);
}
...
async function showSmart(row?: PveRecord) {
  ...
  try {
    smartValues.value = (await getNodeDiskSmart(node, disk)).data || {};
  } finally {
...
function initializeGpt(row?: PveRecord) {
  ...
  Dialog.create({
    title: gettext('Initialize Disk with GPT'),
    message: `${gettext('Initialize Disk with GPT')}: ${disk}`,
    cancel: true,
    persistent: true,
  }).onOk(
    () =>
      void initializeNodeDiskGpt(node, disk).then((result) =>
        openTask(result.data, gettext('Initialize Disk with GPT'), node)
      )
  );
}
...
<template>
  ...
  <DiskSmartDialog
    ...
    :values="smartValues"
    @reload="showSmart({ devpath: smartDisk })"
  />
  <DiskWipeDialog
    v-model="wipeVisible"
    :disk="wipeTarget"
