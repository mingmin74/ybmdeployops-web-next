...
async function create(values: Record<string, unknown>) { if (!operationNode.value) return; saving.value = true; try { const result = await createNodeLvm(operationNode.value, values); createVisible.value = false; openTask(result.data); } finally { saving.value = false; } }
function destroy(row?: PveRecord, node = '') { destroyName.value = textValue(row?.name); operationNode.value = textValue(row?.node || node || props.node); destroyVisible.value = Boolean(operationNode.value && destroyName.value); }
async function confirmDestroy(params: PveRecord) { if (!operationNode.value || !destroyName.value) return; destroying.value = true; try { const result = await deleteNodeLvm(operationNode.value, destroyName.value, params); destroyVisible.value = false; openTask(result.data); } finally { destroying.value = false; } }
