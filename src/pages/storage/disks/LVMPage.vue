...
const actions = computed(() => [
  { name: 'create', label: `${gettext('Create')}: ${gettext('Volume Group')}` },
...
async function loadRows(node: string) {
  ...
  const normalize = (items: PveRecord[], prefix = ''): PveRecord[] =>
    items.map((item, index) => ({
      ...item,
      node,
      name: item.name || `${node}-${prefix}${index}`,
      children: Array.isArray(item.children)
        ? normalize(item.children as PveRecord[], `${prefix}${index}-`)
        : [],
    }));
...
async function create(values: Record<string, unknown>) {
  ...
  saving.value = true;
  try {
    const result = await createNodeLvm(operationNode.value, values);
...
function destroy(row?: PveRecord, node = '') {
  destroyName.value = textValue(row?.name);
...
async function confirmDestroy(params: PveRecord) {
  ...
  try {
    const result = await deleteNodeLvm(operationNode.value, destroyName.value, params);
...
<template>
  <NodeDiskTablePage
    ...
    @action="action"
  >
    <template #body-cell-lvcount="scope">
      <q-td :props="scope">{{ Number(scope.row.__treeLevel || 0) ? '-' : scope.value }}</q-td>
    </template>
    <template #body-cell-free="scope">
      <q-td :props="scope">{{ Number(scope.row.__treeLevel || 0) ? '-' : scope.value }}</q-td>
    </template>
    <template #body-cell-usage="scope">
      <q-td :props="scope">
        <UsageProgress
          v-if="Number(scope.row.__treeLevel || 0) || scope.value !== '-'"
  ...
  <NodeDiskFormDialog
    ...
    :title="`${gettext('Create')}: ${gettext('Volume Group')}`"
    :fields="fields"
