...
const columns: QTableColumn<PveRecord>[] = [
  ...
  {
    name: 'normalized',
    label: gettext('Normalized'),
    field: (row) => row.normalized ?? row.value,
    align: 'right',
  },
...
const attributes = computed(() =>
  Array.isArray(props.values.attributes) ? (props.values.attributes as PveRecord[]) : []
);
...
<template>
  ...
  >
    <UWindow
      :title="`${gettext('S.M.A.R.T. Values')} (${disk})`"
      width="800px"
      :loading="loading"
    >
      <pre
        v-if="isText"
        class="smart-text q-ma-none q-pa-sm"
        >{{ textValue(values.text) }}</pre
      >
      <q-table
        v-else
        flat
