...
async function reload() {
  ...
  try {
    ...
    guests.value = Object.fromEntries(
      (resources.data || [])
...
function goToGuest() {
  ...
  if (!guest) return;
  void router.push({
    name: guest.type === 'qemu' ? 'computer-vm-detail' : 'computer-ct-container-detail',
    params: { node: textValue(guest.node), vmid: textValue(guest.vmid) },
    query: {
      tab: guest.type === 'qemu' ? 'hardware' : 'resources',
      volid: textValue(selectedRow.value?.volid),
    },
  });
}
function remove() {
  ...
  if (guest && (props.shared || textValue(guest.node) === props.node)) {
    Notify.create({
      type: 'negative',
      message: `${gettext("Cannot remove image, a guest with VMID '%s' exists!").replace(
        '%s',
        textValue(row.vmid)
  ...
  Dialog.create({
    title: gettext("Destroy '%s'").replace('%s', textValue(row.volid)),
    message: `${gettext('This action cannot be undone.')} ${gettext(
      'Type the resource ID to confirm'
    )}: ${expectedId}`,
    prompt: { model: '', type: 'text', isValid: (value) => value === expectedId },
    cancel: true,
    persistent: true,
  }).onOk(() => {
    loading.value = true;
    void deleteStorageContent(props.node, props.storage, textValue(row.volid))
      .then((response) => {
    ...
    <template #top>
      <div class="row q-gutter-sm">
        <q-btn
          ...
          size="12px"
          class="u-button"
          :disable="!selectedGuest"
          :label="gettext('Go to Guest')"
          @click="goToGuest"
        />
        <q-btn
          no-caps
