...
async function upload() { if (!file.value || !validFilename.value) return; uploading.value = true; try { await uploadStorageContent(props.node, props.storage, file.value, props.content, (value) => { progress.value = value; }, { filename: filename.value, checksumAlgorithm: checksumAlgorithm.value, checksum: checksum.value }); model.value = false; emit('done'); } finally { uploading.value = false; } }
