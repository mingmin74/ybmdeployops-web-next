...
import { getStorageContent, previewStorageBackupPrune, pruneStorageBackups, updateStorageContent } from '@/api/storageContent';
import { restoreVmBackup, getVmBackupConfiguration } from '@/api/vm';
...
const restoreForm = reactive({ vmid: '', storage: '', bwlimit: '', unique: false, start: false, liveRestore: false });
...
function openRestore() {
  ...
  Object.assign(restoreForm, { vmid: textValue(selectedRow.value?.vmid), storage: '', bwlimit: '', unique: false, start: false, liveRestore: false });
...
async function restore() {
  ...
  loading.value = true;
  try {
    await restoreVmBackup(props.node, { vmid: restoreForm.vmid, archive: selectedVolid.value, ...(restoreForm.storage ? { storage: restoreForm.storage } : {}), ...(restoreForm.bwlimit ? { bwlimit: Number(restoreForm.bwlimit) } : {}), unique: restoreForm.unique ? 1 : 0, ...(restoreForm.start && !restoreForm.liveRestore ? { start: 1 } : {}), ...(restoreForm.liveRestore && isPbs.value ? { 'live-restore': 1 } : {}) }, guestType.value);
...
async function previewPrune() {
  ...
  loading.value = true;
  try { pruneRows.value = (await previewStorageBackupPrune(props.node, props.storage, pruneParams.value)).data || []; }
  finally { loading.value = false; }
}
...
async function prune() {
  if (!guestType.value || !selectedRow.value?.vmid) return;
  loading.value = true;
  try { taskUpid.value = String((await pruneStorageBackups(props.node, props.storage, pruneParams.value)).data || ''); pruneVisible.value = false; taskVisible.value = !!taskUpid.value; await reload(); }
  finally { loading.value = false; }
}
function keepReason(row: PveRecord) {
  ...
  return row.mark === 'keep' ? gettext('Yes') : gettext('No');
}
...
<template>
  ...
  <q-dialog v-model="restoreVisible" persistent><UWindow :title="gettext('Restore')" width="460px" :loading="loading"><div class="q-pa-md q-gutter-md"><q-input v-model="restoreForm.vmid" dense outlined :label="guestType === 'lxc' ? gettext('CT ID') : gettext('VM ID')" /><q-input v-model="restoreForm.storage" dense outlined :label="gettext('Storage')" /><q-input v-model="restoreForm.bwlimit" dense outlined type="number" :label="gettext('Bandwidth Limit')" /><q-checkbox v-model="restoreForm.unique" dense :label="gettext('Unique')" /><q-checkbox v-model="restoreForm.start" dense :disable="restoreForm.liveRestore" :label="gettext('Start after restore')" /><q-checkbox v-if="isPbs" v-model="restoreForm.liveRestore" dense :label="gettext('Live restore')" /></div><template #foot><q-btn v-close-popup no-caps flat :label="gettext('Cancel')" /><q-btn no-caps flat color="primary" :disable="!restoreForm.vmid" :label="gettext('Restore')" @click="restore" /></template></UWindow></q-dialog>
  ...
  <TaskOutputDialog v-model="taskVisible" :node="node" :upid="taskUpid" :title="gettext('Prune')" />
