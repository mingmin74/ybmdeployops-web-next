...
const validRestoreName = computed(
  () => !restoreForm.name.trim() || dnsNamePattern.test(restoreForm.name.trim())
);
const isPbs = computed(
  () => props.storageType === 'pbs' || textValue(selectedRow.value?.format).startsWith('pbs-')
);
...
const pruneLabel = computed(() =>
  selectedRow.value?.vmid && guestType.value
    ? `${gettext('Prune group')} ${guestType.value}/${textValue(selectedRow.value.vmid)}`
    : gettext('Prune group')
);
const pruneTitle = computed(() => {
  const identity = `${guestType.value || ''}/${textValue(selectedRow.value?.vmid)}`;
  return gettext("Prune Backups for '%s' on Storage '%s'")
    .replace('%s', identity)
    .replace('%s', props.storage);
});
const pruneParams = computed(() => {
  const rules = [
    ['keep-last', pruneForm.last],
    ['keep-hourly', pruneForm.hourly],
    ['keep-daily', pruneForm.daily],
    ['keep-weekly', pruneForm.weekly],
    ['keep-monthly', pruneForm.monthly],
    ['keep-yearly', pruneForm.yearly],
  ]
  ...
  return {
    ...
    vmid: textValue(selectedRow.value?.vmid),
    'prune-backups': rules,
  };
});
...
const columns = computed<QTableColumn<PveRecord>[]>(() => {
  const base: QTableColumn<PveRecord>[] = [
    ...
    },
    {
      ...
      align: 'left',
      field: (row) => (Number(row.protected) ? gettext('Yes') : gettext('No')),
      sortable: true,
    },
    {
      name: 'date',
    ...
    base.push(
      {
        ...
        align: 'left',
        field: (row) => formatBackupEncryption(row.encrypted),
        sortable: true,
      },
      {
        name: 'verification',
        ...
        align: 'left',
        field: (row) =>
          formatVerificationState(textValue((row.verification as PveRecord | undefined)?.state)),
        sortable: true,
      }
    );
  return base;
...
async function reload() {
  ...
  try {
    ...
    rows.value = [...(response.data || [])].sort(
      (a, b) => Number(b.vdate || b.ctime || 0) - Number(a.vdate || a.ctime || 0)
    );
    selected.value = [];
  } finally {
    loading.value = false;
...
async function showConfiguration() {
  ...
  try {
    configuration.value = String(
      (await getVmBackupConfiguration(props.node, selectedVolid.value)).data || ''
    );
...
async function saveNotes() {
  ...
  loading.value = true;
  try {
    await updateStorageContent(props.node, props.storage, selectedVolid.value, {
      notes: notes.value,
    });
  ...
  value.split('\n').forEach((line) => {
    ...
    if (key === '#qmdump#map') {
      ...
      allStoragesAvailable =
        allStoragesAvailable &&
        !!storageHint &&
        restoreStorages.value.some((row) => textValue(row.storage) === storageHint);
...
async function loadRestoreDefaults() {
  ...
  try {
    ...
    const [nextId, storages, config] = await Promise.all([
      getNextVmId(),
      getNodeStorage(props.node, content),
      getVmBackupConfiguration(props.node, selectedVolid.value),
    ]);
    restoreForm.vmid = textValue(nextId.data);
    restoreStorages.value = storages.data || [];
    applyRestoreConfiguration(String(config.data || ''));
    if (guestType.value === 'lxc' && !restoreForm.storage)
...
function openFileRestore() {
  if (isPbs.value && selectedVolid.value) fileRestoreVisible.value = true;
}
async function restore() {
  ...
  try {
    ...
    const response = await restoreVmBackup(
      props.node,
      {
        vmid: restoreForm.vmid,
        ...(isLxc
          ? {
              ostemplate: selectedVolid.value,
              restore: 1,
              storage: restoreForm.storage,
              ...(restoreForm.name.trim() ? { hostname: restoreForm.name.trim() } : {}),
              ...(restoreForm.unprivileged === 'keep'
                ? {}
                : { unprivileged: restoreForm.unprivileged === '1' ? 1 : 0 }),
            }
          : {
              archive: selectedVolid.value,
              ...(restoreForm.storage ? { storage: restoreForm.storage } : {}),
              ...(restoreForm.name.trim() ? { name: restoreForm.name.trim() } : {}),
              ...(restoreForm.sockets ? { sockets: Number(restoreForm.sockets) } : {}),
            }),
        ...(restoreForm.bwlimit ? { bwlimit: Number(restoreForm.bwlimit) } : {}),
        unique: restoreForm.unique ? 1 : 0,
        force: 0,
        ...(restoreForm.cores ? { cores: Number(restoreForm.cores) } : {}),
        ...(restoreForm.memory ? { memory: Number(restoreForm.memory) } : {}),
        ...(restoreForm.haManaged ? { 'ha-managed': 1 } : {}),
        ...(restoreForm.start && !restoreForm.liveRestore ? { start: 1 } : {}),
        ...(restoreForm.liveRestore && isPbs.value && !isLxc ? { 'live-restore': 1 } : {}),
      },
      guestType.value
    );
...
async function previewPrune() {
  ...
  loading.value = true;
  try {
    pruneRows.value = addKeepReasons(
      (await previewStorageBackupPrune(props.node, props.storage, pruneParams.value)).data || []
    );
  } finally {
    loading.value = false;
  }
...
async function prune() {
  ...
  loading.value = true;
  try {
    await pruneStorageBackups(props.node, props.storage, pruneParams.value);
...
function keepReason(row: PveRecord) {
  ...
  return row.mark === 'keep'
    ? `${gettext('Yes')} (${textValue(row.keepReason) || gettext('keep-all')})`
    : gettext('No');
}
function addKeepReasons(backups: PveRecord[]) {
  ...
  const hasRule = (rule: string) =>
    Number(pruneForm[rule.replace('keep-', '') as keyof typeof pruneForm]) > 0;
  const nextRule = () => {
    do {
      ruleIndex += 1;
    } while (ruleIndex < rules.length && !hasRule(rules[ruleIndex]!));
...
const pruneColumns = computed<QTableColumn<PveRecord>[]>(() => [
  {
    ...
    field: (row) => (row.ctime ? new Date(Number(row.ctime) * 1000).toLocaleString() : '-'),
    align: 'left',
  },
...
function remove() {
  ...
  Dialog.create({
    ...
    message: gettext('Are you sure to delete [%s]?').replace('%s', selectedVolid.value),
    cancel: true,
    persistent: true,
  }).onOk(
    () =>
      void import('@/api/storageContent')
        .then(({ deleteStorageContent }) =>
          deleteStorageContent(props.node, props.storage, selectedVolid.value)
        )
        .then(reload)
  );
}
...
<template>
  <q-table
    ...
    :selected="selected"
    :filter="filter"
    :pagination="{ page: 1, rowsPerPage: 10, sortBy: 'date', descending: true }"
    ...
    <template #top>
      <div class="row q-gutter-sm">
        ...
        <q-btn
          ...
          size="12px"
          class="u-button"
          :disable="!guestType || !selectedRow?.vmid"
          :label="pruneLabel"
          @click="openPrune"
        />
        <q-btn
          ...
          size="12px"
          class="u-button"
          :color="selectedRow && !Number(selectedRow.protected) ? 'red' : 'grey'"
          :disable="!selectedRow || !!Number(selectedRow.protected)"
  ...
  <q-dialog v-model="configVisible">
    <UWindow
      ...
      width="640px"
    >
      <pre class="q-pa-md backup-config">{{ configuration }}</pre>
    </UWindow>
  </q-dialog>
  <q-dialog
      ...
      <div class="q-pa-md">
        ...
        <div class="row q-col-gutter-md">
          <q-input
            ...
            type="number"
            min="100"
            :label="guestType === 'lxc' ? gettext('CT ID') : gettext('VM ID')"
          />
          <q-input
            ...
            :label="guestType === 'lxc' ? gettext('Hostname') : gettext('Name')"
            :error="!validRestoreName"
          ...
          <q-select
            ...
            emit-value
            map-options
            :clearable="guestType !== 'lxc'"
            :options="
              restoreStorages.map((row) => ({
          ...
          <q-input
            ...
            :class="guestType === 'qemu' ? 'col-4' : 'col-8'"
            dense
            outlined
            type="number"
            min="16"
          ...
          <q-select
            v-if="guestType === 'lxc'"
            v-model="restoreForm.unprivileged"
            class="col-6"
            dense
            outlined
            emit-value
            map-options
            :label="gettext('Privilege Level')"
            :options="[
              { label: gettext('From Backup'), value: 'keep' },
              { label: gettext('Unprivileged'), value: '1' },
              { label: gettext('Privileged'), value: '0' },
            ]"
          />
          <div class="col-6">
            <q-checkbox
          ...
          <div
            v-if="restoreForm.liveRestore && isPbs && guestType === 'qemu'"
            class="col-12 text-warning text-caption"
          >
            {{
              gettext('If live restore fails, data written to the VM during restore may be lost.')
            }}
          </div>
        </div>
      </div>
      <template #foot>
        ...
        <q-btn
          ...
          :disable="
            !restoreForm.vmid ||
            !validRestoreName ||
            (restoreStorageRequired && !restoreForm.storage)
          "
          :label="gettext('Restore')"
          @click="restore"
        />
  ...
  <TaskOutputDialog
    ...
    @finished="reload"
  />
  <StorageFileRestoreDialog
    v-model="fileRestoreVisible"
    :storage="storage"
    :volume="selectedVolid"
  />
</template>
