...
const editableSelectedRole = computed(() =>
  selectedRole.value && !selectedRole.value.special ? selectedRole.value : undefined
);
const canEdit = computed(() => Boolean(editableSelectedRole.value));
const dialogTitle = computed(
  () => `${gettext(formData.action === 'add' ? 'Create' : 'Edit')}: ${gettext('Role')}`
);
const filteredRoles = computed(() => {
  const key = filter.value.trim().toLowerCase();
  return key
    ? roles.value.filter((role) =>
        [role.roleid, role.privs || ''].join(' ').toLowerCase().includes(key)
      )
    : roles.value;
});
...
function resetForm(action: Action) {
  Object.assign(formData, { action, roleid: '', privs: [] });
}
function requiredRule(value: string) {
  return value ? true : gettext('This field is required');
}
function privilegesRule(value: string[]) {
  return value.length ? true : gettext('This field is required');
}
function rowClick(_: Event, row: PveRole) {
...
async function reload() {
  ...
  try {
    ...
    selectedRoles.value = selectedRole.value
      ? roles.value.filter((role) => role.roleid === selectedRole.value?.roleid)
      : [];
  } finally {
    loading.value = false;
  }
...
async function submit() {
  if (
    dialogLoading.value ||
    roleidRef.value?.validate?.() === false ||
    privsRef.value?.validate?.() === false
  )
    return;
  dialogLoading.value = true;
  try {
    ...
    else if (editableSelectedRole.value)
      await updateRole(editableSelectedRole.value.roleid, { privs });
...
function confirmRemove() {
  ...
  Dialog.create({
    ...
    message: gettext('Are you sure to delete [%s]?').replace(
      '%s',
      editableSelectedRole.value.roleid
    ),
    html: true,
...
async function removeSelected() {
  ...
  loading.value = true;
  try {
    await removeRole(editableSelectedRole.value.roleid);
    selectedRoles.value = [];
    await reload();
  } finally {
    loading.value = false;
  ...
  >
    <q-card-section :class="embedded ? 'q-pa-none' : undefined">
      ...
      >
        ...
        <template #body-cell-privs="props">
          <q-td :props="props">
            <div class="role-privileges">
              {{ props.value ? String(props.value).replace(/,/g, ' ') : '-' }}
            </div>
          </q-td>
        </template>
      </q-table>
    ...
    >
      <q-card class="u-window-card role-dialog-card">
