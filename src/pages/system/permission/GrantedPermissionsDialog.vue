...
<template>
  <q-dialog
    v-model="visible"
    class="granted-permissions-dialog"
  >
    <UWindow
      width="800px"
      height="600px"
    ...
    >
      <div class="q-pa-md full-height column">
        <div class="permission-header text-weight-medium text-grey-8 q-pb-sm q-mb-sm">
          <span>{{ gettext('Path') }}/{{ gettext('Permission') }}</span>
          <span>{{ gettext('Propagate') }}</span>
        </div>
        <q-scroll-area class="permission-tree-scroll">
          ...
          >
            <template #default-header="scope">
              <div class="permission-node full-width">
                <span :class="scope.node.type === 'permission' ? 'text-primary' : ''">
                  {{ scope.node.label }}
                </span>
                <span>
                  {{
                    scope.node.type === 'permission'
                      ? scope.node.propagate
                        ? gettext('Yes')
                        : gettext('No')
                      : ''
                  }}
...
.permission-node {
  display: grid;
  grid-template-columns: minmax(0, 6fr) minmax(90px, 1fr);
  column-gap: 16px;
}
.permission-tree-scroll {
  height: 470px;
}
.granted-permissions-dialog :deep(.q-dialog__inner > .u-window-card) {
  overflow: hidden;
}
</style>
