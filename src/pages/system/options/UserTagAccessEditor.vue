...
const registered = computed(() =>
  (Array.isArray(props.registeredTags)
    ? props.registeredTags
    : textValue(props.registeredTags).split(/[;,]/)
  )
    .map((value) => textValue(value).trim())
    .filter(Boolean)
);
const overlappingTags = computed(() =>
  form.tags.split(/[;,\s]+/).filter((tag) => registered.value.includes(tag))
);
...
watch(
  ...
  (value) => {
    ...
    form.tags = Array.isArray(tags)
      ? tags.map((item) => textValue(item)).join(';')
      : textValue(tags);
  },
