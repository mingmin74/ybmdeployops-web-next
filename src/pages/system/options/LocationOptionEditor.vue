...
const props = withDefaults(defineProps<{ modelValue?: PveRecord; showOptionalHint?: boolean }>(), {
...
<template>
  <div class="column q-gutter-sm">
    <q-input
      ...
      :label="gettext('Name')"
      :rules="[
        (value) => !value || !/[=,]/.test(value) || gettext('No comma and equals sign allowed'),
      ]"
    />
    <OptionFormHint v-if="props.showOptionalHint">{{ gettext('Optional') }}</OptionFormHint>
    <q-input
      ...
      :rules="[(value) => inRange(value, -90, 90) || gettext('Invalid latitude')]"
    />
    <q-input
      ...
      :rules="[(value) => inRange(value, -180, 180) || gettext('Invalid longitude')]"
    />
  </div>
</template>
