...
const allowedTags = computed(() => {
  const access =
    props.userTagAccess && typeof props.userTagAccess === 'object'
      ? (props.userTagAccess as Record<string, unknown>)
      : {};
  const value = access['user-allow-list'];
  return (Array.isArray(value) ? value : textValue(value).split(/[;,]/))
    .map((item) => textValue(item))
    .filter(Boolean);
});
const overlappingTags = computed(() =>
  tags.value.split(/[;,\s]+/).filter((tag) => allowedTags.value.includes(tag))
);
...
watch(
  ...
  (value) => {
    tags.value = Array.isArray(value)
      ? value.map((item) => textValue(item)).join(';')
      : textValue(value);
  },
...
<template>
  <div class="column q-gutter-sm">
    <q-input
      ...
      type="textarea"
      :label="gettext('Tags')"
      :rules="[(value) => isValidTagList(value) || gettext('Invalid tag')]"
    />
    <OptionFormHint>{{ gettext('Separate tags with semicolons') }}</OptionFormHint>
    <OptionFormHint v-if="overlappingTags.length">
