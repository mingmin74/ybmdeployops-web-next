...
import { createFirewallAlias, deleteFirewallAlias, getFirewallAliases, updateFirewallAlias } from '@/api/firewall';
...
const columns: QTableColumn<PveRecord>[] = [
  ...
  { name: 'ipversion', label: gettext('IP Version'), align: 'left', field: (row) => row.ipversion || '-', sortable: true },
...
function removeSelected() {
  ...
  Dialog.create({ title: gettext('Confirm'), message: gettext('Are you sure to delete [%s]?').replace('%s', name), cancel: true, persistent: true }).onOk(() => {
    loading.value = true;
    void deleteFirewallAlias(name).then(refreshData).finally(() => {
      loading.value = false;
    });
  });
}
