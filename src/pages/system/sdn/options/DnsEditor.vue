...
<template><q-dialog v-model="visible" persistent><UWindow :title="`${isCreate ? gettext('Add') : gettext('Edit')}: PowerDNS`" width="600px" :loading="loading"><div class="q-pa-md u-dense"><div class="u-border q-pa-md"><div class="row q-col-gutter-lg"><div class="col-6"><q-input v-model="form.dns" dense class="q-field--with-bottom" :disable="!isCreate" maxlength="10" :label="isCreate ? 'ID *' : 'ID'" :error="isCreate && !form.dns.trim()" :error-message="gettext('This field is required')" /><q-input v-model="form.key" dense class="q-field--with-bottom" :label="`${gettext('API Key')} *`" :error="!form.key.trim()" :error-message="gettext('This field is required')" /><q-input v-model="form.fingerprint" dense class="q-field--with-bottom" :label="gettext('Fingerprint')" /></div><div class="col-6"><q-input v-model="form.url" dense class="q-field--with-bottom" :label="`${gettext('URL')} *`" :error="!form.url.trim()" :error-message="gettext('This field is required')" /><q-input v-model="form.ttl" dense type="number" min="0" class="q-field--with-bottom" label="TTL" :error="!ttlValid" :error-message="gettext('Invalid TTL')" /></div></div></div></div><template #foot><q-btn v-close-popup no-caps outline size="12px" class="u-button" :label="gettext('Cancel')" /><q-btn no-caps flat size="12px" class="bg-primary text-grey-1 u-button" :label="isCreate ? gettext('Create') : gettext('OK')" @click="save" /></template></UWindow></q-dialog></template>
