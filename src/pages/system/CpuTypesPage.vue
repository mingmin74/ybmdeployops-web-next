...
<template>
  <div class="q-ma-md">
    <q-table flat row-key="cputype" selection="single" table-header-class="u-table-header" :rows="rows"
      :columns="columns" :selected="selected" :loading="loading"
      :pagination="{ sortBy: 'cputype', descending: false, rowsPerPage: 10 }" :rows-per-page-options="[10]"
      :no-data-label="gettext('No custom CPU models configured')" @update:selected="selected = [...$event]"
      @row-dblclick="(_, row) => void edit(row)"><template #top>
        <div class="row q-gutter-sm">
          <q-btn no-caps outline size="12px" color="primary" class="u-button" :label="gettext('Add')"
            @click="add" /><q-btn no-caps outline size="12px" color="primary" class="u-button"
            :disable="selected.length !== 1" :label="gettext('Edit')" @click="() => void edit()" /><q-btn no-caps
            outline size="12px" class="u-button" :color="selected.length === 1 ? 'red' : 'grey'"
            :disable="selected.length !== 1" :label="gettext('Remove')" @click="remove" /><q-space /><q-btn no-caps
            outline size="12px" color="primary" class="u-button" :label="gettext('Reload')" @click="reload" />
        </div>
      </template></q-table>
  </div>
  <q-dialog v-model="editorVisible" persistent>
    <UWindow :title="`${editingId ? gettext('Edit') : gettext('Add')}: ${gettext('CPU Type')}`" width="800px"
      :loading="saving">
      <div class="q-pa-md u-dense">
        <div class="u-border q-pa-md q-gutter-sm">
          <q-input v-model="form.cputype" dense maxlength="40" :disable="Boolean(editingId)" :label="gettext('Name')"
            :error="!configIdValid" :error-message="form.cputype.trim()
                ? gettext('Only letters, numbers, underscores, dots and hyphens are allowed')
                : gettext('This field is required')
              " /><q-select v-model="form.reportedModel" dense use-input clearable emit-value map-options hide-selected
            :loading="selectorLoading" :options="baseModelOptions" :label="gettext('Base Model')"
            hint="CPU model the rest of the configuration is based on."
            :error="!editingId && !form.reportedModel.trim()" :error-message="gettext('This field is required')"
            @filter="filterBaseModels" /><q-input v-model="form.hvVendorId" dense maxlength="12"
            :label="gettext('Hyper-V Vendor')" />
          <div>
            <div class="q-field__label">{{ gettext('Physical Address Bits') }}</div>
            <q-radio v-model="form.physBits" dense val="" :label="gettext('Default from QEMU')" /><q-radio
              v-model="form.physBits" dense val="host" :label="gettext('Inherit from host CPU')" /><q-radio
              v-model="form.physBits" dense :val="form.physBits && form.physBits !== 'host' ? form.physBits : '48'"
              :label="gettext('Custom value')" /><q-input v-if="form.physBits !== 'host' && form.physBits !== ''"
              v-model="form.physBits" dense type="number" min="8" max="64" :label="gettext('Custom value')"
              :error="!physBitsValid" :error-message="gettext('Value must be 8-64')" />
          </div>
          <q-checkbox v-model="form.hidden" dense :label="gettext('Hide Hypervisor')" />
          <div>
            <div class="q-field__label">{{ gettext('Extra CPU flags') }}</div>
            <q-table flat dense hide-bottom row-key="name" class="vm-cpu-flags-table" :rows="cpuFlags"
              :columns="flagColumns" :loading="selectorLoading" :pagination="{ rowsPerPage: 0 }"><template
                #body-cell-state="props"><q-td :props="props"><q-btn-toggle :model-value="props.row.state" dense no-caps
                    unelevated toggle-color="primary" :options="[
                      { label: gettext('Off'), value: '-' },
                      { label: gettext('Default'), value: '=' },
                      { label: gettext('On'), value: '+' },
                    ]" @update:model-value="
                      setCpuFlagState(textValue(props.row.name), $event)
                      " /></q-td></template></q-table>
          </div>
        </div>
      </div>
      <template #foot><q-btn v-close-popup no-caps outline size="12px" class="u-button"
          :label="gettext('Cancel')" /><q-btn no-caps flat size="12px" class="bg-primary text-grey-1 u-button"
          :disable="!canSave" :label="editingId ? gettext('OK') : gettext('Create')" @click="save" /></template>
    </UWindow>
  </q-dialog>
</template>
